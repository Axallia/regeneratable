...
$execute if score *regeneratable.rcah_local.removed_entry regeneratable.success matches 0 \
    run execute if score *regeneratable.min_message_severity regeneratable.const matches ..2 run tellraw @a "§6<warn> [regeneratable.entry_remover] No entry was found at $(x) $(y) $(z)"